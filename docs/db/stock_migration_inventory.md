...
**Date** : 2025-12-27  
**Version** : 1.0  
**Objectif** : Inventaire exhaustif des usages des vues legacy stock pour planifier la migration vers les vues snapshot canoniques
...
**Total** : 10 occurrences à analyser
...
1. **Module Sorties** : `stock_actuel` dans formulaire
   - **Fichier** : `lib/features/sorties/providers/sortie_providers.dart`
   - **Action** : Remplacer par `v_stock_actuel_snapshot`
   - **Impact** : Affichage stock réel temps présent dans formulaire
...
3. **Stocks Repository** : `v_citerne_stock_actuel` dans `fetchTotauxStocks()`
   ...
   - **Action** : Remplacer par agrégation depuis `v_stock_actuel_snapshot` ou `v_kpi_stock_global`
   - **Impact** : Totaux stocks par dépôt corrects

### Priorité 🟡 Moyenne (Providers legacy)

4. **Module Citernes** : Providers legacy `stock_actuel`
   - **Fichiers** :
     - `lib/features/citernes/providers/citerne_providers.dart` (déjà @Deprecated)
     - `lib/features/citernes/data/citerne_service.dart` (déjà @Deprecated)
   - **Action** : Vérifier que ReceptionService n'en dépend plus, puis supprimer
   - **Impact** : Faible (déjà remplacés)

5. **Owner totals** : `v_stock_actuel_owner_snapshot`
   - **Fichier** : `lib/data/repositories/stocks_kpi_repository.dart` (`fetchDepotOwnerTotals`)
   - **Action** : Créer vue owner snapshot-based ou agréger depuis `v_stock_actuel_snapshot`
   - **Impact** : Breakdown propriétaire avec stock réel temps présent

### Priorité 🟢 Basse (Déjà deprecated)

6. **Alias deprecated** : `fetchDepotOwnerStocksFromSnapshot()`
