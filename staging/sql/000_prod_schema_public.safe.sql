...
CREATE VIEW public.cours_route WITH (security_invoker='on') AS
 SELECT c.id,
    c.fournisseur_id,
    c.depot_destination_id,
    c.produit_id,
    TRIM(BOTH FROM concat_ws(' • '::text, NULLIF(c.plaque_camion, ''::text), NULLIF(c.plaque_remorque, ''::text))) AS plaques,
    COALESCE(c.chauffeur_nom, ''::text) AS chauffeur,
    COALESCE(c.transporteur, ''::text) AS transporteur,
    c.volume,
    c.statut,
    c.date_chargement AS date,
    c.created_at
...
CREATE VIEW public.stock_actuel AS
 SELECT DISTINCT ON (sj.citerne_id, sj.produit_id) sj.citerne_id,
    sj.produit_id,
    sj.date_jour,
    sj.stock_ambiant,
    sj.stock_15c
   FROM public.stocks_journaliers sj
  ORDER BY sj.citerne_id, sj.produit_id, sj.date_jour DESC;
...
CREATE VIEW public.v_citerne_stock_actuel AS
 ...
 SELECT last_rows.citerne_id,
    last_rows.produit_id,
    max(last_rows.date_jour) AS date_jour,
    sum(last_rows.stock_ambiant) AS stock_ambiant,
    sum(last_rows.stock_15c) AS stock_15c
   FROM last_rows
  GROUP BY last_rows.citerne_id, last_rows.produit_id;
...
CREATE VIEW public.v_kpi_stock_global AS
 SELECT s.depot_id,
    TRIM(BOTH FROM s.depot_nom) AS depot_nom,
    s.produit_id,
    s.produit_nom,
    (max(s.updated_at))::date AS date_jour,
    sum(s.stock_ambiant) AS stock_ambiant_total,
    sum(s.stock_15c) AS stock_15c_total,
    sum(
        CASE
            WHEN (s.proprietaire_type = 'MONALUXE'::text) THEN s.stock_ambiant
            ELSE (0)::double precision
        END) AS stock_ambiant_monaluxe,
    sum(
        CASE
            WHEN (s.proprietaire_type = 'MONALUXE'::text) THEN s.stock_15c
            ELSE (0)::double precision
        END) AS stock_15c_monaluxe,
    sum(
        CASE
            WHEN (s.proprietaire_type = 'PARTENAIRE'::text) THEN s.stock_ambiant
            ELSE (0)::double precision
        END) AS stock_ambiant_partenaire,
    sum(
        CASE
            WHEN (s.proprietaire_type = 'PARTENAIRE'::text) THEN s.stock_15c
            ELSE (0)::double precision
        END) AS stock_15c_partenaire
   FROM public.v_stock_actuel_snapshot s
  GROUP BY s.depot_id, (TRIM(BOTH FROM s.depot_nom)), s.produit_id, s.produit_nom;
...
CREATE VIEW public.v_stock_actuel AS
 SELECT v_stocks_snapshot_corrige.depot_id,
    v_stocks_snapshot_corrige.citerne_id,
    v_stocks_snapshot_corrige.produit_id,
    v_stocks_snapshot_corrige.proprietaire_type,
    v_stocks_snapshot_corrige.stock_ambiant_corrige AS stock_ambiant,
    v_stocks_snapshot_corrige.stock_15c_corrige AS stock_15c,
    v_stocks_snapshot_corrige.last_movement_at,
    v_stocks_snapshot_corrige.updated_at,
    v_stocks_snapshot_corrige.stock_ambiant_base,
    v_stocks_snapshot_corrige.stock_15c_base,
    v_stocks_snapshot_corrige.delta_ambiant_total,
    v_stocks_snapshot_corrige.delta_15c_total
...
--
-- Name: log_actions read logs; Type: POLICY; Schema: public; Owner: -
--
