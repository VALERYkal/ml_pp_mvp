...
final receptionsTodayParamProvider = Provider<({String? depotId, String dayYmd})>((ref) {
...
final receptionsKpiProvider =
    FutureProvider.family<ReceptionsStats, ({String? depotId, String dayYmd})>((ref, p) async {
      final repo = ref.watch(receptionsRepoProvider);
      // dayYmd est un 'YYYY-MM-DD' UTC
      final parts = p.dayYmd.split('-');
      final dayUtc = DateTime.utc(int.parse(parts[0]), int.parse(parts[1]), int.parse(parts[2]));

      final r = await repo.statsJour(dayUtc: dayUtc, depotId: p.depotId);
      return ReceptionsStats(nbCamions: r.nbCamions, volAmbiant: r.volAmbiant, vol15c: r.vol15c);
    });
