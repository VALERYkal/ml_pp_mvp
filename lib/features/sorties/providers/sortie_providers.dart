...
final sortieServiceProvider = Provider<SortieService>((ref) {
  return SortieService(); // Utilise le constructeur par défaut avec Supabase.instance.client
});

final sortieDraftServiceProvider = Provider<SortieDraftService>((ref) {
  return SortieDraftService(Supabase.instance.client);
});

/// Liste des sorties (lecture simple MVP)
final sortiesPageProvider = StateProvider<int>((ref) => 0);
final sortiesPageSizeProvider = StateProvider<int>((ref) => 25);

final sortiesListProvider = FutureProvider<List<Map<String, dynamic>>>((
  ref,
) async {
...
final clientsListProvider = FutureProvider<List<Map<String, String>>>((
  ref,
) async {
  final res = await Supabase.instance.client
      .from('clients')
...
final citernesByProduitWithStockProvider =
    FutureProvider.family<List<CiterneWithStockForSortie>, String>((
      ref,
      produitId,
    ) async {
      ...
      final stocks =
          await supabase
                  ...
                  .inFilter('citerne_id', citerneIds)
                  .eq('produit_id', produitId)
              as List;

      final byCiterne = {
        for (final s in stocks) (s['citerne_id'] as String): s,
      };
