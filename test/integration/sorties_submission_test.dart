...
void main() {
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      debugPrint(
        '🔍 DEBUG: Produit chip sélectionné: ${produitChipWidget.selected}',
      );
      ...
      final citerneRadioWidget = tester.widget<RadioListTile<String>>(
        citerneRadioParent,
      );
      debugPrint(
        '🔍 DEBUG: Citerne radio value: ${citerneRadioWidget.value}, groupValue: ${citerneRadioWidget.groupValue}',
      );
      ...
      expect(
        ...
        findsNothing,
        reason: 'Le formulaire ne doit pas être en état de chargement',
      );
      ...
      expect(
        ...
        isTrue,
        reason:
            'Les champs de saisie (TextFormField ou TextField) doivent être présents dans le formulaire',
      );
      ...
      expect(
        mesuresCardText,
        findsOneWidget,
        reason: 'La carte Mesures & Calculs doit être présente',
      );
      ...
      expect(
        mesuresCard,
        findsOneWidget,
        reason: 'La Card Mesures & Calculs doit être présente',
      );
      ...
      expect(
        mesuresTextFormFields,
        findsNWidgets(4),
        reason:
            'Il doit y avoir exactement 4 TextFormField numériques dans la carte Mesures & Calculs',
      );
      ...
      final avantWidget0 = tester.widget<TextFormField>(
        mesuresTextFormFields.at(0),
      );
      debugPrint(
        '🔍 DEBUG: Après enterText "0" dans TextFormField[0]: "${avantWidget0.controller?.text ?? ""}"',
      );
      ...
      // Vérifier que la valeur a été appliquée
      final apresWidget1 = tester.widget<TextFormField>(
        mesuresTextFormFields.at(1),
      );
      debugPrint(
        '🔍 DEBUG: Après enterText "100" dans TextFormField[1]: "${apresWidget1.controller?.text ?? ""}"',
      );
      ...
      final avantFinal = tester.widget<TextFormField>(
        mesuresTextFormFields.at(0),
      );
      final apresFinal = tester.widget<TextFormField>(
        mesuresTextFormFields.at(1),
      );
      final tempFinal = tester.widget<TextFormField>(
        mesuresTextFormFields.at(2),
      );
      final densFinal = tester.widget<TextFormField>(
        mesuresTextFormFields.at(3),
      );
      debugPrint(
        '🔍 DEBUG: Valeurs finales - Index avant: "${avantFinal.controller?.text ?? ""}", Index après: "${apresFinal.controller?.text ?? ""}", Temp: "${tempFinal.controller?.text ?? ""}", Dens: "${densFinal.controller?.text ?? ""}"',
      );
      ...
      try {
        ...
        debugPrint(
          '⚠️ DEBUG: Impossible de forcer les membres privés (limitation Dart): $e',
        );
        debugPrint(
          '⚠️ DEBUG: Les interactions UI devraient avoir rempli ces champs',
        );
      }
      ...
      debugPrint(
        '🔍 DEBUG: callsCount avant submitSortieForTesting: $callsBefore',
      );
