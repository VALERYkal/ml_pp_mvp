...
class _SpySortieService extends SortieService {
  ...
  _SpySortieService({this.exceptionToThrow}) : super(SupabaseClient('http://localhost', 'anon'));
...
void main() {
  testWidgets('SortieFormScreen affiche le formulaire et peut soumettre', (tester) async {
    ...
    await tester.pumpWidget(
      ProviderScope(
        overrides: [
          ...
          refs.produitsRefProvider.overrideWith((ref) => Future.value(fakeProduits)),
          refs.citernesActivesProvider.overrideWith((ref) => Future.value(fakeCiternes)),
    ...
    expect(find.byType(TextField), findsAtLeastNWidgets(4)); // Au moins 4 TextField (indices, T°, densité)
  ...
  group('SortieFormScreen - Validation & comportement', () {
    Widget createTestWidget({
      ...
      final fakeProduits = produits ?? [
        refs.ProduitRef(id: 'p1', code: 'ESS', nom: 'Essence'),
      ];
      final fakeCiternes = citernes ?? [
        refs.CiterneRef(
          id: 'cit1',
          nom: 'Citerne Test 1',
          produitId: 'p1',
      ...
      final fakePartenaires = partenaires ?? [
        {'id': 'pa1', 'nom': 'Partenaire X'}
      ];
      ...
      return ProviderScope(
        overrides: [
          ...
          P.partenairesListProvider.overrideWith((ref) async => fakePartenaires),
        ],
        child: const MaterialApp(home: SortieFormScreen()),
      );
    }

    testWidgets('Le bouton "Enregistrer" est désactivé quand le formulaire est incomplet', (tester) async {
      // Arrange
      ...
      if (allFilledButtons.evaluate().isNotEmpty) {
        ...
        expect(saveButton.onPressed, isNull, reason: 'Le bouton doit être désactivé quand le formulaire est incomplet');
      } else {
    ...
    testWidgets('Le bouton "Enregistrer" devient actif quand tous les champs obligatoires sont remplis', (tester) async {
      // Arrange
      await tester.pumpWidget(createTestWidget());
      await tester.pumpAndSettle();
      ...
      final buttonFinder = filledButtonFinder.evaluate().isNotEmpty 
          ? filledButtonFinder 
          : (allFilledButtons.evaluate().isNotEmpty ? allFilledButtons : null);
      
      if (buttonFinder != null && buttonFinder.evaluate().isNotEmpty) {
        final saveButton = tester.widget<FilledButton>(buttonFinder.first);
        expect(saveButton.onPressed, isNotNull, reason: 'Le bouton doit être actif quand tous les champs sont remplis');
      } else {
    ...
    testWidgets('En cas de formulaire invalide, SortieService ne doit PAS être appelé', (tester) async {
      // Arrange
      final spyService = _SpySortieService();
      ...
      expect(spyService.called, isFalse, reason: 'Le service ne doit pas être appelé si le formulaire est invalide');
    });

    testWidgets('Si SortieService lève une exception, un SnackBar d\'erreur est affiché', (tester) async {
      // Arrange
      ...
      if (saveButton.onPressed != null) {
        ...
        await tester.pump(); // Deuxième pump pour permettre à l'exception de se propager
        await tester.pump(const Duration(milliseconds: 500)); // Attendre que le SnackBar s'affiche
      } else {
      ...
      expect(spyService.called, isTrue, reason: 'Le service doit avoir été appelé');
    });
  });
}
